...
export const DelegateTaskTool = Tool.make("delegate_task", {
  description:
    "Ask a helper using an exact taskType from orchestrator_capabilities and a self-contained task brief. The owner's task table selects model, effort and family. Do not supply target, role, runtimeMode or interactionMode. For other work use the current family's native helpers. mode='async' returns immediately and delivers completion to the parent; mode='wait' waits up to timeoutMs without cancelling the helper.",
