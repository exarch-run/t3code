...
import { ChatAttachment, QuestionResponse } from "@t3tools/contracts";
...
const makeProjectionThreadMessageRepository = Effect.gen(function* () {
  ...
  const upsertProjectionThreadMessageRow = SqlSchema.void({
    ...
    execute: (row) => {
      ...
      const nextQuestionResponseJson =
        row.questionResponse !== undefined ? JSON.stringify(row.questionResponse) : null;
      return sql`
        ...
        VALUES (
          ...
          COALESCE(
            ${nextQuestionResponseJson},
            (
              SELECT question_response_json
              FROM projection_thread_messages
              WHERE message_id = ${row.messageId}
            )
          ),
        ...
        DO UPDATE SET
          ...
          question_response_json = COALESCE(
            excluded.question_response_json,
            projection_thread_messages.question_response_json
          ),
